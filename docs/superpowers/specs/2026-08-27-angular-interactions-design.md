...
| Server state               | **TanStack Query**                             | Consistent with the rest of the Angular app. The detail page stays on `httpResource` (a deliberate contrast with phase 1); interaction mutations invalidate **only the interactions list** — the server never writes the parent hiring-process row, so the detail record cannot go stale, and the header's count is derived from that same list. |
