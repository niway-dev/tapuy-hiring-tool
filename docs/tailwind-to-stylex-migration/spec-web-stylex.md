...
| D7  | `stylex.when.*` is allowed only where state genuinely lives on another element (4 sites today, see `05-porting-conventions.md` §7.5). Everything else is a JS conditional from Base UI state or an explicit style on the child                                                                                                             | Keeps styles local and typed; avoids selector magic                                                                                                                                                                                                 |
...
| `group-hover:` `peer-disabled:`                     | `stylex.when.ancestor(":hover")` / `when.siblingBefore(":disabled")` + `defineMarker()` (own `*.stylex.ts` file, per `label.stylex.ts`) — the only sanctioned uses |
